...
namespace PTEJsonO
{
    public partial class frmMain : Form
    {
        ...
        private void cmd103_Click(object sender, EventArgs e)
        {
            var Content = File.ReadAllLines(@"E:\Desktop\103.txt").ToList();
            Console.Write(Content);
            var fitContent = new List<string>();
            var fitMain = new List<string>();
            string[] wordList;
            ...
            for (var i = 0; i < Content.Count; i++)
                if (true)
                {
                    ...
                    var tempOption = "";
                    //var tempAnswer = "";
                    var tempMain = tempArray[0].Replace("completionText:\"", "");
                    tempMain = tempMain.Replace("\",\"serviceActive\":[{\"selectWords", "")
                        .Replace(@"\u2018", "'").Replace(@"\u2019", "'").Replace(@"\u2013", "-").Replace(@"\u2014", "-")
                        .Replace(@"\u201c", "\"").Replace(@"\u201d", "\"").Replace(@"\n", "").Replace("''", "\"")
                        .Replace(@"\", "").Replace("^", " ");
                    Regex.Replace(tempMain, @"\b\s+\b", " ");
            ...
            var fitAnswer = new List<string>();
             */
            var fileName = "103-" + DateTime.Now.ToString("yyyy-MM-dd--hh-mm-ss") + ".xlsx";
            var file = new FileInfo(fileName);
            using (var package = new ExcelPackage(file))
            {
                var worksheet = package.Workbook.Worksheets.Add("105 - " + DateTime.Now.ToShortDateString());
